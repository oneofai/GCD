...
- (instancetype)initSerialWithLabel:(NSString *)label {
    ...
    self = [super init];
    if (self) {
        self.dispatchQueue = dispatch_queue_create(label ? [label UTF8String] : nil, DISPATCH_QUEUE_SERIAL);
    }
...
- (instancetype)initConcurrentWithLabel:(NSString *)label {
    self = [super init];
    if (self) {
        self.dispatchQueue = dispatch_queue_create(label ? [label UTF8String] : nil, DISPATCH_QUEUE_CONCURRENT);
    }
    return self;
}

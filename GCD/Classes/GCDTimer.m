...
- (void)countDownEvent:(void (^)(float))block completionEvent:(dispatch_block_t)completionEvent cancelEvent:(dispatch_block_t)cancelEvent timeIntervalWithSecs:(float)secs totalSecs:(float)totalSecs {
    
    NSParameterAssert(block);
    __block float timeout = totalSecs;
    dispatch_source_set_timer(self.dispatchSource, dispatch_walltime(DISPATCH_TIME_NOW, 0), secs * NSEC_PER_SEC, 0);
